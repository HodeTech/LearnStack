...
The **commit-msg** hook (same activation) checks the subject against
[Git Workflow § Commits](../docs/standards/14-git-workflow.md#commits): Conventional
Commits shape and 72 characters, judged on the subject git will store. CI's `meta` job
runs the **same hook** on every commit of a pull request, so the point is not a second
rule but where the failure lands: CI catches a bad subject after a push, and a subject is
only fixable by rewriting history; locally it is a retry. A merge is skipped, and git's
autosquash markers (`fixup!`, `squash!`, `amend!`) pass locally so a fixup workflow works
— CI refuses them, so squash before review. Git's own `Revert "…"` subject is refused
everywhere; § Reverts says what a revert's subject is. The `ADR:` and `Module:` trailers
are deliberately not checked: whether a commit owes one depends on judgement a hook does
not have, and a hook that guessed is a hook people disable.
