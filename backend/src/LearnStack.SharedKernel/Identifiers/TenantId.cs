...
/// *value* is fixed nowhere, and the only thing that consumes it is
/// <c>audit_log</c>, which
...
/// identifier for a table that does not exist yet; Packet 9 chooses it, with the
/// schema that stores it. Note that ADR-0036 forbids the sentinel on a different
