...
The table below is the current, accurate state as of 2026-08-28, at HEAD with
[Phase 02a](../roadmap/phase-02a-kernel-tenancy.md) Packets 0–3, 3b, 4, 5 and 6
shipped.
...
| 08 | [Localization](08-localization.md) | **Adopted** | Packet 6 shipped `tenant_locales` and the slug schema; the i18n runtime lands in [Phase 04](../roadmap/phase-04-cms-media-pages.md). Nothing enforces "exactly one default locale per tenant" yet; [Packet 7](../roadmap/phase-02a-kernel-tenancy.md) does, with a partial unique index `UNIQUE (tenant_id) WHERE is_default` plus an aggregate-level guard for the error message. |
...
| 21 | [Architecture Tests Catalogue](21-architecture-tests-catalogue.md) | **Active** | Twenty-nine test methods run in the architecture assembly and seven further implemented rules run beside it, three of them against an applied schema; the catalogue's own per-row status column distinguishes those from the registered-but-unimplemented majority. |
