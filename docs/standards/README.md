...
| 21 | [Architecture Tests Catalogue](21-architecture-tests-catalogue.md) | **Active** | A hundred and thirty test methods — 174 cases — run in the architecture assembly, and forty-one further implemented rules run beside it in the unit, integration and frontend suites, most of them where they can actually fail: against an applied schema, a real host, or a built page. `Every_Implemented_Rule_Names_A_Test_That_Exists` holds each Implemented entry to a method of that name, so the catalogue's per-row status column is checked rather than trusted. |
