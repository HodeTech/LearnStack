...
| 21 | [Architecture Tests Catalogue](21-architecture-tests-catalogue.md) | **Active** | Twenty-nine test methods run in the architecture assembly and seven further implemented rules run beside it, four of them against an applied schema; the catalogue's own per-row status column distinguishes those from the registered-but-unimplemented majority. |
