...
The table below is the current, accurate state as of 2026-08-08, at HEAD with
[Phase 02a](../roadmap/phase-02a-kernel-tenancy.md) Packets 0–3 shipped.
...
| 03 | [Frontend Coding](03-frontend-coding.md) | **Adopted** | ESLint and TypeScript strict mode are configured, but `apps/web` is a scaffold with no components and no tests. First real code: [Phase 02d](../roadmap/phase-02d-walking-skeleton.md). |
...
| 06 | [Testing](06-testing.md) | **Active** | Unit, architecture and contract suites run in CI, and the meta-test proves the architecture suite can fail. The **integration** suite does not: the backend job filters it out (`FullyQualifiedName!~LearnStack.Tests.Integration`) and its own job is `if: false` until Phase 02a Packet 7 lands the first isolation test. |
...
| 14 | [Git Workflow](14-git-workflow.md) | **Active** | Conventional Commits, hooks and required checks are live. The branch-protection settings that contradict this standard are reconciled in Packet 3b. |
