...
| 21 | [Architecture Tests Catalogue](21-architecture-tests-catalogue.md) | **Active** | Eighty-eight test methods — 113 cases — run in the architecture assembly, and eleven further implemented rules run beside it in the unit and integration suites, several against an applied schema; the catalogue's own per-row status column distinguishes those from the rules still registered. |
