...
| **`ISecretProvider`** | Interface for secret reads. `EnvironmentSecretProvider` today; the Vault-backed implementation when secrets must rotate without a redeploy or a non-development deployment exists. Secret namespace `learnstack/{deployment}/{module}/{key}`. |
