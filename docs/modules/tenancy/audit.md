...
**A cell carries its slug and, where the row is not joined by request type, one marker**
...
`tenancy.entitlement.refresh`, written by `IEntitlementProvider.RefreshAsync`; and the two
