...
>   `source is not a directory`, so the hook aborted **every** commit for anyone who
>   installed the pinned version.
> - The same hook's `pnpm exec eslint` runs from `frontend/`, where `eslint` is not a
>   dependency — and ESLint 9 does not read the `.eslintrc.cjs` the app still uses, so
