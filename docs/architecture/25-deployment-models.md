...
- **Secret store** — `ConfigurationSecretProvider` today in every mode; the Vault-backed
  provider is demand-gated to Phase 11 behind `ISecretProvider`, triggered when a production secret must rotate without a redeploy, or more than one operator needs access to production secrets.
