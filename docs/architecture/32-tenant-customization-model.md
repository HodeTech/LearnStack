...
| A `$ref` resolving outside its document, or a **non-productive** `$ref` cycle | **Not admitted** | Both are measured to survive the builder. An unresolvable fragment raises only when an entry is validated against it, so the author's mistake lands on someone else's save; a cycle whose hops are pure indirection overflows the stack during evaluation — which .NET cannot catch, so one tenant's schema ends the process for every tenant the pod serves. Productive recursion stays legal: a `$ref` under `properties` consumes an instance level per hop and terminates ([ADR-0043 § 3](../decisions/0043-customization-payload-validation.md)) |
