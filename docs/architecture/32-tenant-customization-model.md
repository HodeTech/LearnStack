...
|---|---|---|
| `$ref` / `$defs` nesting depth inside one schema | 5 **schema** levels | JSON Schema validators are recursive, and it is a schema level that costs a frame. The unit was not stated and matters: one schema level costs **two** JSON levels — an applicator keyword then a name (`properties`, then `word`) — so the writer checks raw JSON depth against twelve, the same bound counted in the unit it can see. Counting schema positions instead would mean enumerating every applicator keyword, and one missed from that list is a hole rather than a stricter bound |
| A `$ref` resolving outside its document, **any** `$ref` cycle, or a reference graph costing more than 1,000 expansions | **Not admitted** | All three are measured to survive the builder. An unresolvable fragment raises only when an entry is validated against it, so the author's mistake lands on someone else's save; a cycle overflows the stack during evaluation — which .NET cannot catch, so one tenant's schema ends the process for every tenant the pod serves; and an **acyclic** graph of twenty doubling `$defs` entries, 1,401 bytes, expands to 2^20 visits at 5.7 GB. Recursion is given up rather than sorted into productive and not: telling the terminating case apart means enumerating every in-place applicator, and getting that list wrong costs a process rather than an answer ([ADR-0043 Amendment 2](../decisions/0043-customization-payload-validation.md)) |
...
| Serialised size of one content entry | 1 MB | PostgreSQL will happily TOAST more; the browser will not happily render it. Enforced by `IJsonSchemaValidator` itself and not only by the HTTP body limit, because the seeder and the bulk importer reach the validator without passing through a request. Measured at exactly this cap: a hundred properties each applying one shared `$defs` shape, against a 1 MiB instance, costs **742 ms and 1.6 GB of allocation** in one call — the price of the limit, stated so the next person to change it knows what they are buying |
| Serialised size of one customization row (`json_schema`, a rule `body`, a taxonomy band's `metadata`) | 256 KB | Keeps a definition inside one round trip. [Phase 05 § Limits](../roadmap/phase-05-education-learning-content.md) owns it as a `LimitKeys` entry; it is enforced from [Packet 8](../roadmap/phase-02a-kernel-tenancy.md), which is where the first `json_schema` is written — a limit that arrives after its first writer is a limit that arrives after the rows it was meant to bound |
...
| `$id`, `$anchor`, `$dynamicRef`, `$dynamicAnchor` | **Not admitted** | `$id` is a schema-resource identity the evaluator records; under a shared registry a tenant's `$id` becomes process-global state and locks another tenant out of saving ([ADR-0043 § 3](../decisions/0043-customization-payload-validation.md)). `$anchor` is the same keyword class — a name a fragment could resolve through — and is refused with them |
| `$ref` form | Fragment-only (`#…`) | An absolute `$ref` is a resolution attempt against something outside the document |
| Root of a `json_schema` | A JSON **object** — not `true`, `false` or `{}` — declaring a non-empty `properties` | All three are valid 2020-12 schemas; the first and third switch validation off for every entry of that type, and § 8.1's read-time structural pass has no field list to walk. "Object schema" here means the schema document is an object, **not** that it declares `type: "object"` — a root `type` is left to the author, and a root that says `type: "string"` is a schema no entry will satisfy rather than a hazard the platform has to refuse |
...
10. **A tenant schema is admitted only through the four gates**, and the LearnStack
   ...
   `$anchor` / `$dynamicRef` / `$dynamicAnchor`, fragment-only `$ref` costed against an
   expansion cap, no regex keywords, nothing a `jsonb` column refuses — is the
